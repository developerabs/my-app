...
@section('content')
    @component('backend.layouts.partials.header')
        ...
        @endslot
        @slot('subtitle')
            {{ __('file.title.asset_register_desc') }}
        @endslot
        @slot('button')
            <a href="{{ route('assets.index') }}" class="btn btn-primary"><i class="fa-solid fa-list me-1"></i>
                {{ __('file.button.list') }} {{ __('file.asset') }}</a>
    ...
    <form action="{{ route('assets.register.store') }}" method="POST" id="asset_register_form">
        @csrf
        <div class="card">
                ...
                <div class="row align-items-end">
                    ...
                    <div class="col-md-2 mb-3">
                        ...
                        <select name="entry_type" id="entry_type" class="form-select select-picker" required>
                            @forelse ([\App\Enums\AssetEntryType::OPENING, \App\Enums\AssetEntryType::PURCHASE] as $entryType)
                                <option value="{{ $entryType->value }}">{{ ucfirst($entryType->value) }}</option>
                            @empty
                    ...
                    <!-- Payment Account (Hidden by default, shown on purchase) -->
                        ...
                        <select name="payment_account_id" id="payment_account_id" class="form-select select-picker">
                            ...
                            @forelse ($paymentAccounts as $account)
                                <option value="{{ $account->id }}"
                                    {{ ($general_settings['default_acc'] ?? '') == $account->id ? 'selected' : '' }}>
            ...
            <div class="card-body">
                    ...
                    <table class="table table-bordered align-middle" id="asset_items_table">
                        <thead>
                            <tr>
                                ...
                                <th width="15%" class="supplier_column" style="display: none;">{{ __('Supplier') }}
                                ...
                                <th width="10%" class="supplier_column" style="display: none;">{{ __('Paid Amount') }}
                                    <span class="text-danger">*</span>
                                </th>
                        ...
                        <tbody>
                            <tr data-row-index="0">
                                <td>
                                    <select name="items[0][asset_id]" class="form-select item-asset asset-select" required>
                                ...
                                <td>
                                    <input type="number" step="any" name="items[0][quantity]"
                                        class="form-control form-control-sm quantity" value="1" required>
                                </td>
                                <td>
                                    <input type="number" step="any" name="items[0][unit_cost]"
                                        class="form-control form-control-sm unit_cost" required placeholder="0.00">
                                </td>
                                <td>
                                    <input type="number" step="any" name="items[0][total_cost]"
                                        class="form-control form-control-sm total_cost" readonly placeholder="0.00">
                                </td>
                                <td class="supplier_column" style="display: none;">
                                    <input type="number" step="any" name="items[0][paid_amount]"
                                        class="form-control form-control-sm paid_amount" placeholder="0.00">
                                </td>
                                <td>
                                    <input type="number" step="any" name="items[0][salvage_value]"
                                        class="form-control form-control-sm" value="0">
                                </td>
                                <td>
                                    <input type="text" name="items[0][useful_life]" class="form-control form-control-sm"
                                        placeholder="e.g. 5 Years">
                                </td>
                                <td>
                                    <input type="text" name="items[0][depreciation_start_date]"
                                        class="form-control form-control-sm depreciation_start_date" placeholder="YYYY-MM-DD">
                                </td>
                ...
                <div class="d-flex justify-content-between align-items-center mt-3">
                    ...
                    <div class="text-end">
                        <h4 class="fw-bold mb-0 text-dark">
                            {{ __('Grand Total') }}: <span id="grand_total" class="text-primary">0.00</span>
                        </h4>
                    </div>
                </div>
                ...
                <div class="text-end mt-4">
                    <button type="submit" class="btn btn-primary px-4">
                        <i class="fa-solid fa-save me-1"></i> {{ __('Save Register') }}
                    </button>
                </div>
            </div>
        </div>
...
@push('js')
    <script>
        ...
        $('#add_row_btn').click(function() {
            ...
            // Global supplier select initialization call
