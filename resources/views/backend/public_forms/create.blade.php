...
@section('title', __('file.create_public_form'))
@section('content')
    @component('backend.layouts.partials.header')
        @slot('title') {{ __('file.create_public_form') }} @endslot
        @slot('subtitle') {{ __('file.build_compact_public_form') }} @endslot
        @slot('button')
        ...
        @endslot
    @endcomponent
    <form action="{{ route('public-forms.store') }}" id="publicFormCreate" method="POST" enctype="multipart/form-data">
        @csrf 
        @include('backend.public_forms._form')
    </form>
@endsection
@push('js')
<script>
    $('#publicFormCreate').on('submit', function(e) {
            e.preventDefault();
            let form = $(this);
