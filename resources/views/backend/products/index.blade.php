    ...
    <script>
        function viewProduct(id) {
            ...
            $.get(url, function(product) {
                ...
                let pharmacyItemDetails = '';
                @if (is_feature_active('pharmacy'))
                    let displayDrugType = '-';
                    if (product.drug_type) {
                        // Convert the first letter to uppercase and slice the rest of the string (ucfirst equivalent in JS)
                        displayDrugType = product.drug_type.charAt(0).toUpperCase() + product.drug_type.slice(1);
                    }
                    pharmacyItemDetails = `
                        <div class="d-flex justify-content-between mb-1 pb-1 border-bottom border-white">
                            <span class="text-muted">Generic Name:</span> <span class="fw-bold">${product.generic?.name || '-'}</span>
                        </div>
                        <div class="d-flex justify-content-between mb-1 pb-1 border-bottom border-white">
                            <span class="text-muted">Drug Type:</span> <span class="fw-bold">${displayDrugType}</span>
                        </div>`;
                @endif
                ...
                let html = `
                    <div class="container-fluid p-0">
                        ...
                        <div class="row g-3 mb-3">
                            ...
                            <!-- ডান কলাম -->
                            <div class="col-md-8 product-detail-container" id="product-container-${product.id}">

                                <!-- This wrapper contains all top-level info that will be hidden in Full View mode -->
                                <div class="product-main-meta-wrapper collapse show">
                                    <!-- Product Type Badge, SKU, and Internal Code Info -->
                                    ...
                                    <!-- Meta Data Segment: Brand Label and Category Badges Stack -->
                                    <div class="d-flex flex-wrap gap-3 align-items-center mb-3 pb-3 border-bottom border-gray-100" style="font-size: 12px;">
                                        <div>
                                            ...
                                            <span class="badge bg-light text-dark border ms-1 fw-normal px-2 py-1">${product.brand?.name || 'N/A'}</span>
                                        </div>
                                    ...
                                    <!-- Pricing Matrix Blocks: Sale, Wholesale, and Cost configurations -->
                                    ...
                                    <!-- Contextual Short Description Callout -->
                                    ...
                                    <!-- Collapsible Accordion Block: Specifications, Warranties, and Dropshipping Configurations -->
                                ...
                                <!-- Advanced Dynamic Stock Matrix: Tab Navigation UI -->
                                ${(() => {
                                    ...
                                    let hasImeis = product.imeis && product.imeis.length > 0;
                                    ...
                                    return `
                                    <div class="w-100 mb-4 product-stock-matrix-wrapper">
                                        <div class="d-flex border-bottom border-gray-300 mb-2 tab-navigation-row">
                                            <ul class="nav nav-tabs border-0 custom-pos-tabs" id="productTab-${product.id}" role="tablist" style="font-size: 13px; margin-bottom: -1px;">
                                                ...
                                                ${hasImeis ? `
                                                <li class="nav-item" role="presentation">
                                                    <button class="nav-link py-2 px-3 bg-transparent" 
                                                            id="imei-tab-${product.id}" data-bs-toggle="tab" data-bs-target="#imei-pane-${product.id}" 
                                                            type="button" role="tab" aria-controls="imei-pane-${product.id}" aria-selected="false">
                                                        IMEI Tracking
                                                    </button>
                                                </li>` : ''}
                                            </ul>
                                        ...
                                        <style>
                                            ...
                                            /* CSS rule to restrict height in normal layout view mode */
                                            .limited-table-view {
                                                max-height: 260px; 
                                                overflow-y: auto;
                                            }
                                            /* Remove height threshold in full view mode */
                                            .product-detail-container.is-full-view .limited-table-view {
                                                max-height: none !important;
                                                overflow-y: visible !important;
                                            }
                                        </style>

                                        <!-- Dynamic Tab Execution Content Windows -->
                                        <div class="tab-content" id="productTabContent-${product.id}">
                                            ...
                                            <div class="tab-pane fade show active" id="variants-pane-${product.id}" role="tabpanel" aria-labelledby="variants-tab-${product.id}">
                                                <div class="table-responsive rounded border border-gray-200">
                                                    <table class="table table-sm table-striped align-middle m-0" style="font-size: 12px;">
                                                        ...
                                                        <tbody>
                                                            ${product.all_variants.map((v, index) => {
                                                                ...
                                                                let isHiddenRow = index >= 5;
                                                                // Unique target class for variants
                                                                let collapseClass = isHiddenRow ? `collapse extra-variants-${product.id} bg-light-subtle` : '';
                                                                ...
                                                                ${hasNestedBatches ? `
                                                                <tr class="collapse ${isHiddenRow ? `extra-variants-${product.id}` : ''}" id="nested-batch-${v.id}">
                                                                                ...
                                                                                <tbody>
                                                                                    ${v.batches.map(b => `
                                                                                        <tr>
                                                                                            ...
                                                                                            <td class="text-muted">${formatedDate(b.expiry_date) || 'No Expiry'}</td>
                                                            ...
                                                            ${product.all_variants.length > 5 ? `
                                                            <tr>
                                                                <td colspan="6" class="text-center p-2 bg-light-subtle">
                                                                    <button class="btn btn-sm btn-outline-primary fw-bold px-3 structure-toggle-btn" 
                                                                            type="button" 
                                                                            aria-expanded="false"
                                                                            onclick="toggleViewBtnText(this, '${product.id}', 'extra-variants')">
                                                                        View Full (${product.all_variants.length - 5} More) ▾
                                                                    </button>
                                                                </td>
                                                            </tr>
                                                            ` : ''}
                                                    ...
                                                    <table class="table table-sm table-striped align-middle m-0" style="font-size: 12px;">
                                                        ...
                                                        <tbody>
                                                            ${product.batches.map((b, index) => {
                                                                let isHiddenRow = index >= 5;
                                                                // Unique target class for standalone batches
                                                                let collapseClass = isHiddenRow ? `collapse extra-batches-${product.id}` : '';
                                                            ...
                                                            <tr>
                                                                <td colspan="5" class="text-center p-2 bg-light-subtle">
                                                                    <button class="btn btn-sm btn-outline-primary fw-bold px-3 structure-toggle-btn" 
                                                                            type="button" 
                                                                            aria-expanded="false"
                                                                            onclick="toggleViewBtnText(this, '${product.id}', 'extra-batches')">
                                                                        View Full (${product.batches.length - 5} More) ▾
                                                                    </button>
                                                                </td>
                                                            </tr>
                                                            ` : ''}
                                            ...
                                            <div class="tab-pane fade ${activeTab === 'branches' ? 'show active' : ''}" id="branches-pane-${product.id}" role="tabpanel" aria-labelledby="branches-tab-${product.id}">
                                                <div class="table-responsive rounded border border-gray-200">
                                                    <table class="table table-sm table-striped align-middle m-0" style="font-size: 12px;">
                                                        ...
                                                        <tbody>
                                                            ${product.aggregated_branch_stocks.map((bs, index) => {
                                                                let isHiddenRow = index >= 5;
                                                                // Unique target class for branches
                                                                let collapseClass = isHiddenRow ? `collapse extra-branches-${product.id}` : '';

                                                                return `
                                                                <tr class="${collapseClass}">
                                                                    <td class="ps-3 fw-bold text-dark">${bs.branch_name}</td>
                                                                    <td class="text-muted">${bs.location || '-'}</td>
                                                                    <td class="text-center fw-bold text-success">${bs.baseStock}</td>
                                                                </tr>
                                                                `;
                                                            }).join('')}

                                                            ${product.aggregated_branch_stocks.length > 5 ? `
                                                            <tr>
                                                                <td colspan="3" class="text-center p-2 bg-light-subtle">
                                                                    <button class="btn btn-sm btn-outline-primary fw-bold px-3 structure-toggle-btn" 
                                                                            type="button" 
                                                                            aria-expanded="false"
                                                                            onclick="toggleViewBtnText(this, '${product.id}', 'extra-branches')">
                                                                        View Full (${product.aggregated_branch_stocks.length - 5} More) ▾
                                                                    </button>
                                                                </td>
                                                            </tr>
                                                            ` : ''}
                                            ...
                                            ${hasImeis ? `
                                            <div class="tab-pane fade" id="imei-pane-${product.id}" role="tabpanel" aria-labelledby="imei-tab-${product.id}">
                                                <div class="table-responsive rounded border border-gray-200">
                                                    <table class="table table-sm table-striped align-middle m-0" style="font-size: 12px;">
                                                        <thead class="table-light text-uppercase" style="font-size: 11px;">
                                                            <tr>
                                                                <th class="ps-3 py-2">IMEI NUMBER</th>
                                                                <th>BRANCH</th>
                                                                <th>BATCH</th>
                                                                <th>STATUS</th>
                                                            </tr>
                                                        ...
                                                        <tbody>
                                                            ${product.imeis.map((imei, index) => {
                                                                let isHiddenRow = index >= 5;
                                                                let collapseClass = isHiddenRow ? `collapse extra-imeis-${product.id}` : '';
                                                                
                                                                return `
                                                                <tr class="${collapseClass}">
                                                                    <td class="ps-3 fw-bold text-dark">${imei.imei_number}</td>
                                                                    <td class="text-muted">${imei.branch_name || 'N/A'}</td>
                                                                    <td class="text-muted">${imei.batch_no || 'N/A'}</td>
                                                                    <td>
                                                                        <span class="badge ${imei.status === 'sold' ? 'bg-danger-subtle text-danger' : 'bg-success-subtle text-success'}">
                                                            ...
                                                            ${product.imeis.length > 5 ? `
                                                            <tr>
                                                                <td colspan="4" class="text-center p-2 bg-light-subtle">
                                                                    <button class="btn btn-sm btn-outline-primary fw-bold px-3" 
                                                                            type="button" onclick="toggleViewBtnText(this, '${product.id}', 'extra-imeis')">
                                                                        View Full (${product.imeis.length - 5} More) ▾
                                                                    </button>
                                                                </td>
                                                            </tr>` : ''}
                                                        </tbody>
        ...
        /**
         * Global Handler Function to handle Full View switching
         * Place this logic inside your main scripts file or block scope.
         * 
         * @param {string|number} productId - Target element identifier
         * @param {boolean} shouldExpand - State driver toggle condition
         */
        function toggleProductFullView(productId, shouldExpand) {
            const container = document.getElementById(`product-container-${productId}`);
            if (!container) return;

            const metaWrapper = container.querySelector('.product-main-meta-wrapper');
            const fullViewHeader = container.querySelector('.full-view-header');
            const footerActionContainer = container.querySelector('.full-view-action-container');

            if (shouldExpand) {
                // Step 1: Append state hook flag to container 
                container.classList.add('is-full-view');
                
                // Step 2: Hide standard contextual details
                if (metaWrapper) metaWrapper.classList.add('d-none');
                if (footerActionContainer) footerActionContainer.classList.add('d-none');
                
                // Step 3: Show specialized table actions bar
                if (fullViewHeader) {
                    fullViewHeader.classList.remove('d-none');
                    fullViewHeader.classList.add('d-flex');
                }
            } else {
                // Reverse execution matrix flow back to absolute defaults
                container.classList.remove('is-full-view');
                
                if (metaWrapper) metaWrapper.classList.remove('d-none');
                if (footerActionContainer) footerActionContainer.classList.remove('d-none');
                
                if (fullViewHeader) {
                    fullViewHeader.classList.add('d-none');
                    fullViewHeader.classList.remove('d-flex');
                }
            }
        }

        function toggleViewBtnText(button, productId, targetClassName) {
            // Select the specific elements based on the passed class name
            const extraRows = document.querySelectorAll(`.${targetClassName}-${productId}`);
            const productMainContent = document.querySelector('.product-main-meta-wrapper');
            
            if (!productMainContent) return;

            // Initialize or get Bootstrap collapse instance for the product meta wrapper
            let bsMetaCollapse = bootstrap.Collapse.getInstance(productMainContent);
            if (!bsMetaCollapse) {
                bsMetaCollapse = new bootstrap.Collapse(productMainContent, { toggle: false });
            }

            // Save the original text count dynamically using dataset
            if (!button.dataset.remainingCount) {
                const match = button.innerText.match(/\((\d+)\s+More\)/i);
                button.dataset.remainingCount = match ? match[1] : '0';
            }
            const remainingCount = button.dataset.remainingCount;

            // Determine current state
            const isExpanding = button.getAttribute('aria-expanded') === 'false' || !button.hasAttribute('aria-expanded');

            if (isExpanding) {
                // 1. Smoothly show the target rows
                extraRows.forEach(row => {
                    let bsRow = bootstrap.Collapse.getInstance(row);
                    if (!bsRow) bsRow = new bootstrap.Collapse(row, { toggle: false });
                    bsRow.show();
                });

                // 2. Smoothly hide the main meta content
                bsMetaCollapse.hide();
                ...
                // 1. Smoothly hide the target rows
                extraRows.forEach(row => {
                    let bsRow = bootstrap.Collapse.getInstance(row);
                    if (bsRow) bsRow.hide();
                });

                // 2. Smoothly show the main meta content
                bsMetaCollapse.show();

                // 3. Update button
                button.setAttribute('aria-expanded', 'false');
                button.innerHTML = `View Full (${remainingCount} More) ▾`;
            }
        }
    </script>
@endpush
