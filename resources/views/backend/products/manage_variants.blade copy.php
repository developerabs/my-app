                    ...
                    <div class="col-xl-8">
                        ...
                        <div id="attribute-wrapper">
                            <div class="row g-2 attribute-row align-items-center mb-2">
                                <div class="col-3">
                                    <select name="attributes[0][name]" class="form-control form-control-sm attr-name-select"
                                ...
                                <div class="col-1 text-end">
                                    <button type="button" class="btn btn-danger-light btn-sm remove-attr-btn"><i
...
@push('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            const baseSkuCode = $('#base-sku').text().trim();
            const productCode = "{{ str_pad($product->code ?? $product->id, 5, '0', STR_PAD_LEFT) }}";
            const baseCost = "{{ $product->cost ?? 0 }}";
            const basePrice = "{{ $product->price ?? 0 }}";
            const baseWholesale = "{{ $product->wholesale_price ?? 0 }}";
        ...
        <div class="row g-2 attribute-row align-items-center mb-2 animate__animated animate__fadeIn">
            ...
            function renderVariants(combinations) {
                ...
                combinations.forEach((variant, index) => {
                    // If only one attribute, variant will be a string, convert to array
                    let variantArray = Array.isArray(variant) ? variant : [variant];
                    let variantName = variantArray.join('-').toUpperCase().replace(/\s+/g, '');
                    let fullItemCode = `${productCode}-${variantName}`.replace(/-+/g, '-');
                    let cleanSku = `${baseSkuCode}-${variantName}`.replace(/-+/g, '-');
                    let displayTitle = variantArray.join(' / ');

                    let html = `
            <tr class="animate__animated animate__fadeIn">
                <td class="ps-3 variant-info-box">
                    <div class="fw-bold text-primary fs-14 mb-2">${displayTitle}</div>
                    <div class="mb-1"><span class="text-muted small uppercase">SKU:</span> <b>${cleanSku}</b></div>
                    <div><span class="text-muted small uppercase">CODE:</span> <b class="text-dark">${fullItemCode}</b></div>
                ...
                <td class="pricing-group">
                    <div class="d-flex flex-column gap-2">
                        <div class="input-group">
                            ...
                            <input type="number" name="variants[${index}][cost]" class="form-control" value="${baseCost}" step="0.01">
                        </div>
                        <div class="input-group">
                            <span class="input-group-text">SELL PRICE</span>
                            <input type="number" name="variants[${index}][price]" class="form-control" value="${basePrice}" step="0.01">
                        </div>
                        <div class="input-group">
                            <span class="input-group-text">WHOLESALE</span>
                            <input type="number" name="variants[${index}][wholesale_price]" class="form-control" value="${baseWholesale}" step="0.01">
                        </div>
                    </div>
                </td>
                <td>
                ...
                <td class="text-center">
                    <button type="button" class="btn btn-sm btn-icon btn-danger-light remove-row"><i class="ri-delete-bin-line"></i></button>
                </td>
            </tr>`;
