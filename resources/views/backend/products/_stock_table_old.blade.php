@php
    // Filter existing stocks for this specific variant or for single product (null variant)
    $filteredStocks = $existingStocks->where('product_variant_id', $variantId);

    // Safely detect if variant object is empty and fall back to base product cost/prices
...
<div class="border rounded overflow-hidden">
                ...
                @if ($filteredStocks->count() > 0)
                    @foreach ($filteredStocks as $stock)
                        @php
                            ...
                            $expiryDate =
                                $stock->batch && $stock->batch->expiry_date
                                    ? date('d-m-Y', strtotime($stock->batch->expiry_date))
                                    : '';
                            $masterBarcode = '';
                            if ($stock->batch) {
                                // Adjust 'barcode_type' condition if you save IMEI and Master Barcode separately.
                        ...
                        <tr class="stock-row" data-row-id="{{ $rowId }}">
                            <td>
                                <input type="hidden" name="stocks[{{ $rowId }}][id]"
                                    value="{{ $stock->id }}">
                                <select name="stocks[{{ $rowId }}][branch_id]"
                                    class="form-select select2-simple" required>
                            ...
                            <td><input type="text" name="stocks[{{ $rowId }}][batch_no]" class="form-control"
                                    value="{{ $batchNo }}" placeholder="Batch#"></td>
                            <td>
                                @if ($product->has_expire_date)
                                    <input type="text" name="stocks[{{ $rowId }}][expiry_date]"
                                        class="form-control expiry-datepicker" value="{{ $expiryDate }}" required>
                                @else
                                    <input type="text" class="form-control expiry-datepicker" disabled
                                        placeholder="N/A">
                                    <input type="hidden" name="stocks[{{ $rowId }}][expiry_date]"
                                        value="0000-00-00">
                                @endif
                            </td>
                            <td><input type="number" name="stocks[{{ $rowId }}][cost]"
                                    class="form-control text-end"
                                    value="{{ $stock->batch ? $stock->batch->cost : ($stock->prices ? $stock->prices->cost : $rowCost) }}" step="any"></td>
                            <td><input type="number" name="stocks[{{ $rowId }}][price]"
                                    class="form-control text-end"
                                    value="{{ $stock->batch ? $stock->batch->price : ($stock->prices ? $stock->prices->price : $rowPrice) }}" step="any">
                            </td>
                            <td><input type="number" name="stocks[{{ $rowId }}][wholesale_price]"
                                    class="form-control text-end"
                                    value="{{ $stock->batch ? $stock->batch->wholesale_price : ($stock->prices ? $stock->prices->wholesale_price : $rowWholesale) }}"
                                    step="any"></td>

                            {{-- Quantity Column - Clean & Standard --}}
                            <td class="bg-light-transparent" style="min-width: 180px;">
                                <div class="d-flex flex-wrap gap-1 justify-content-center">
                                    @php $sortedUnits = get_sorted_units($product->unit_details ?? []); @endphp
                                    @foreach ($sortedUnits as $unit)
                                        <div class="compound-unit-item" style="width: 50px;">
                                            <input type="text"
                                                ...
                                                data-unit-id="{{ $unit['unit_id'] }}" placeholder="0"
                                                style="font-size: 11px;">
                                            <div class="unit-label text-center bg-light border border-top-0"
                                                style="font-size: 9px; padding: 1px 0;">{{ $unit['short_name'] }}</div>
                                        </div>
                                    @endforeach
                                </div>
                                <input type="hidden" name="stocks[{{ $rowId }}][quantity]"
                                    class="final-base-qty" value="{{ $stock->quantity }}">
                                <input type="hidden" name="stocks[{{ $rowId }}][product_variant_id]" value="{{ $stock->product_variant_id }}">
                                <div class="text-center mt-1">
                                    <span class="badge bg-primary-transparent total-display"
                                        style="font-size: 10px;">Total: {{ number_format($stock->quantity, 2) }}</span>
                                </div>
                            </td>
                            <td class="text-center">
                                <button type="button"
                                    class="btn btn-icon btn-sm btn-danger-transparent remove-stock-group"><i
                        ...
                        @if ($product->has_imei)
                            @php
                                $dbImeiCount = ($stock->batch && $stock->batch->imeis) ? $stock->batch->imeis->count() : 0;
                                echo $dbImeiCount;
                                    ...
                                    <div class="card card-body shadow-none border-dashed mb-0 p-2 bg-white">
                                        <div class="d-flex align-items-center justify-content-between border-bottom pb-1 mb-2">
                                            <span class="fw-bold text-dark fs-11 text-uppercase">
                                                <i class="fa-solid fa-barcode text-primary me-1"></i> IMEI / Serial Inventory Allocation</span>
                                            <button type="button"
                                                class="btn btn-xs btn-outline-primary add-single-imei px-2 py-0"
                                                style="font-size: 10px;"><i class="fa-solid fa-plus me-1"></i>Add Manual
                                                Field</button>
                                        </div>
                                        <div class="imei-wrapper d-flex flex-wrap gap-2 pt-1">
                                            @if ($stock->batch && $stock->batch->imeis)
                                                @foreach ($stock->batch->imeis as $imei)
                                                    <div
                                                        class="d-flex align-items-center dynamic-imei-group bg-light rounded p-1 border">
                                                        <input type="text"
                                                        ...
                                                        <button type="button"
                                                            class="btn btn-xs text-danger remove-imei-field ms-1 p-0"><i
                                            ...
                                            @if ($targetQuantity > $dbImeiCount)
                                                @for ($i = 0; $i < ($targetQuantity - $dbImeiCount); $i++)
                                                    <div class="d-flex align-items-center dynamic-imei-group bg-light rounded p-1 border">
                                                        ...
                                                        <button type="button" class="btn btn-xs text-danger remove-imei-field ms-1 p-0"><i class="fa-solid fa-circle-xmark"></i></button>
                                                    </div>
                                                @endfor
                                            @endif
                                        </div>
                                        <div class="imei-error text-danger fs-11 fw-bold mt-2 ps-1"
                                            style="display: none;"><i
                                                class="fa-solid fa-circle-exclamation me-1"></i>Duplicate IMEI detected
                                            in this stock row!</div>
                                    </div>
                                </td>
                            </tr>
                        @endif
    ...
    <div class="p-2 bg-light border-top">
        <button type="button" class="btn btn-sm btn-primary-transparent add-stock-row"
            data-variant-id="{{ $variantId }}" data-cost="{{ $rowCost }}" data-price="{{ $rowPrice }}"
