...
@section('title')
    {{ __('file.title.assign_permissions') }} - {{ $general_settings['site_title'] ?? $general_settings['company_name'] ?? 'SheraziPOS' }}
@endsection
...
@section('content')
    ...
    @component('backend.layouts.partials.header')
        @slot('title'){{ __('file.title.assign_permissions') }}@endslot
        @slot('subtitle'){{ __('file.title.assign_permission_desc') }}@endslot
    @endcomponent
                ...
                <div class="card-body">
                    <form action="{{ route('assign-permissions', $role->id) }}" method="POST">
                                                        ...
                                                        <div class="col-12 col-md-3 col-lg-2">
                                                            <div class="form-check">
                                                                ...
                                                                <label class="form-check-label ms-2" for="perm-{{ $permission->id }}">
                                                                    {{ Str::title(str_replace('_', ' ', $permission->display_custom_name)) }}
                                                                </label>
                                                            </div>
                                                        </div>
                                                    @endforeach
                        ...
                        <div class="mt-4 text-start">
                            ...
                            <a href="{{ route('roles-permissions') }}" class="btn btn-secondary px-4">
