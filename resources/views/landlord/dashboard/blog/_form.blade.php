...
<div class="row">
    ...
    <div class="col-md-2 mb-3">
        ...
        <select class="form-select" id="status" name="status" required>
            @php $status = old('status', $blog->status ?? 'published'); @endphp
...
<div class="mb-3">
    ...
    <div id="content">{!! old('content', $blog->content ?? '') !!}</div>
    <textarea id="source-container" style="display:none; width:100%; height:300px;"></textarea>

    <input type="hidden" name="content" id="content_input" value="{{ old('content', $blog->content ?? '') }}">
</div>

{{-- Image --}}
        <div class="col-md-6">
            <label class="form-label">{{ __('file.field.image') }}</label>
            ...
            <img id="image_preview"
                 src="{{ isset($blog) && $blog->image ? asset('storage/'.$blog->image) : asset('images/preview_image.png') }}"
    ...
    <div class="col-md-6">
        <div class="col-12 mb-3">
            ...
            <input type="text" class="form-control" id="meta_title" name="meta_title"
                   value="{{ old('meta_title', $blog->meta['meta_title'] ?? '') }}">
        </div>
        <div class="col-12 mb-3">
            <label for="meta_keywords" class="form-label">{{ __('file.field.meta_keywords') }}</label>
            <input type="text" class="form-control" id="meta_keywords" name="meta_keywords"
                   value="{{ old('meta_keywords', $blog->meta['meta_keywords'] ?? '') }}">
        </div>
    </div>
    <div class="col-md-6 mb-3">
        <label for="meta_description" class="form-label">{{ __('file.field.meta_description') }}</label>
        <textarea class="form-control" id="meta_description" name="meta_description" rows="5">{{ old('meta_description', $blog->meta['meta_description'] ?? '') }}</textarea>
    </div>
</div>
