...
@section('content')
    ...
    <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
        <div>
            <h4 class="mb-0">{{__('file.title.create_page')}}</h4>
            <p class="mb-0 text-muted">{{__('file.title.create_page_desc')}}</p>
        </div>
        <div>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('landlord.pages.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf

                        @include('landlord.dashboard.page._form')
                        <div class="text-end">
                            <button type="submit" class="btn btn-primary px-4">💾 {{__('file.button.create')}} {{__('file.page')}}</button>
                        </div>
                    </form>
                </div>
            </div>
