...
class LeadSourceDataTable extends BaseDataTable
{
   protected string $tableId = 'lead-source-table';
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($row) {
                $user = Auth::user();
                $actions = '';
                $actions .= '<a href="javascript:void(0)" onclick="editLeadSource(\'' . (string) $row->id . '\')" class="btn btn-primary btn-sm" title="Edit"><i class="fa-solid fa-pen-to-square"></i></a>';

                $url = route('lead-sources.destroy', (string) $row->id);
                $actions .= '<button type="button" data-url="' . $url . '" data-table-id="#lead-source-table" data-item="' . $row->name . '" data-name="Lead Source" class="btn btn-danger btn-sm delete-btn" title="Delete"><i class="fa-solid fa-trash"></i></button>';
    ...
    public function query(LeadSource $model): QueryBuilder
    {
        ...
        if (request()->filled('status')) {
            $query->where('lead_sources.is_active', request('status'));
        }
    ...
    protected function getCustomButtons(): array
    {
        return [
            [
                ...
                'attr' => [
                    'id' => 'bulk-delete-btn',
                    'data-url' => route('lead-sources.bulk-delete'),
                ],
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('is_active')->data('is_active')->name('lead_sources.is_active')->title(__('file.table.status')),
    ...
    {
        return 'Lead_Source_' . date('YmdHis');
    }
}
