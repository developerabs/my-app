...
class CurrencyDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($row) {
                $edit = '<a href="javascript:void(0)" onclick="editCurrency(' . $row->id . ')" class="btn  btn-primary"><i class="fa-solid fa-pen-to-square"></i></a>';
                $delete = '<button type="button" onclick="deleteCurrency(' . $row->id . ')" class="btn btn-danger"><i class="fa-solid fa-trash"></i></button>';
                if(!$row->is_default){
                    return $edit . ' ' . $delete;
                }else{
                    return $edit;
    ...
    public function getColumns(): array
    {
         return [
            ...
            Column::make('symbol')->title(__('file.table.symbol')),
