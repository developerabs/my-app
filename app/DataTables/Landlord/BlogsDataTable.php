     ...
     public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('title',function($row){
                return '<a href="'.url('blogs/'.$row->slug).'" target="_blank">'.$row->title.'</a>';
            })
            ->editColumn('image',function($row){
                return '<img src="'.($row->image ? asset('storage/'.$row->image) : asset('images/preview_image.png')).'" alt="'.e($row->title).'" style="max-height: 50px;">';
            })
            ...
            ->addColumn('action', function ($row) {
                $editBtn = '<a href="' . route('landlord.blogs.edit', $row->id) . '" class="btn btn-sm btn-primary"><i class="fa-solid fa-pen-to-square me-1"></i></a>';
                $deleteBtn = '<a href="javascript:void(0);" onclick="deleteBlog('.$row->id.')" class="btn btn-sm btn-danger"><i class="fa-solid fa-trash me-1"></i></a>';
                return $editBtn . ' ' . $deleteBtn;
            })
            ->rawColumns(['DT_RowIndex', 'title', 'image', 'status', 'action'])
            ->setRowId('id');
    }
