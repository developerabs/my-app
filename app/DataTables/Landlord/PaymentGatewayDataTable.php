...
class PaymentGatewayDataTable extends DataTable
{
     ...
     *
     * @param QueryBuilder<PaymentGateway> $query Results from query() method.
     */
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('image', function ($row) {
                $image = $row->logo ? $row->logo_url : asset('assets/landlord/img/no-image.png');
                return '<img src="'.e($image).'" alt="'.e($row->display_name).'" class="img-fluid" style="max-height:30px;">';
            })
            ->editColumn('status', function ($row) {
                return $row->is_active ? '<span class="badge bg-success">'.__('file.active').'</span>' : '<span class="badge bg-danger">'.__('file.inactive').'</span>';
            })
            ->addColumn('action', function ($row) {
                $edit = '<a href="javascript:void(0)" onclick="editGateway('.$row->id.')" class="btn  btn-primary"><i class="fa-solid fa-pen-to-square"></i></a>';
                $delete = '<button type="button" onclick="deleteGateway('.$row->id.')" class="btn btn-danger"><i class="fa-solid fa-trash"></i></button>';
                return $edit.' '.$delete;
            })
     ...
     *
     * @return QueryBuilder<PaymentGateway>
     */
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('image')->title(__('file.table.image'))->orderable(false)->searchable(false)->addClass('text-start'),
            Column::make('display_name')->title(__('file.table.name'))->addClass('text-start'),
            Column::make('status')->title(__('file.table.status'))->sortable(false)->addClass('text-start'),
